...
def get_det_loss_mrcnn(data,model,pargs,p_img,label_true_forloss,conf_weight=1,iou_weight=1,iou_clamp=0.4):
    ...
    if total_dim!=0:
        ...
        if len(iou1.shape)==1:
            ...
            mean_iou=torch.mean(torch.clamp(torch.cat(mean_iou_list,dim=0),iou_clamp,1.0)-iou_clamp)
    else:
    ...
    return conf_weight*conf_loss+iou_weight*mean_iou,0
...
def train_patch_stage1():
    ...
    img_path = 'replace_with_your_npy_file'#load the last stage 1 patch from ensemble_advtshirt_stage1.py
    cloth = torch.from_numpy(np.load(img_path)).float()
    ...
    for epoch in range(1, 600 + 1):
        ...
        for m_index in range(len(model_list)):
            if m_index!=optimize_index:
                continue
            if epoch==1 or epoch%ap_eval_epoch==0 or (epoch<100 and epoch%30==0):
        ...
        for i_batch in range(total_image_num):
            ...
            for i in range(len(crop_size_list[counter])):
                cur_cs=int(np.random.rand()*(int(crop_size_list[counter][i]*t_scale)-int(crop_size_list_min[counter][i])*t_scale)+int(crop_size_list_min[counter][i]*t_scale))
                cur_crop_size.append(cur_cs)
            adv_patch_crop=adv_patch_t
            ...
            adv_patch_tps, _ = tps.tps_trans(adv_patch_crop, max_range=tps_strength, canvas=0.5)
            if adv_patch_tps.shape[0]==0 or label_true_forloss.shape[1]==0:
                continue
            ...
            adv_batch_t = patch_transformer(adv_patch_tps, label_true_forloss_combined, pargs.img_size, do_rotate=True, rand_loc=False,
                                            pooling=pargs.pooling, old_fasion=kwargs['old_fasion'],ratio=cur_crop_size[1]/cur_crop_size[0],rotate_strength=rotate_strengths[counter],shift=shift_strengths[counter], t_scale=t_scale)
            ...
            if args.net=='yolov2':
                ...
                if cur_model_index==0:
                    ...
                    det_loss, valid_num = get_det_loss_mrcnn(data,model_list[cur_model_index],pargs,p_img_batch,label_true_forloss,conf_weight=conf_iou_weight[counter][0],iou_weight=conf_iou_weight[counter][1])
...
def train_patch():
    ...
    balance_limits=[[9,9,9,9,10,12,13],
                    [9,9,9,9,10,12,13]]
    change_epoch=100
    balance_index=0
    ...
    for epoch in range(1, 1400+1):
        ...
        for i_batch in range(total_image_num):
            ...
            adv_patch_tps, _ = tps.tps_trans(adv_patch_crop_copy, max_range=tps_strength, canvas=0.5)
            if adv_patch_tps.shape[0]==0 or label_true_forloss.shape[1]==0:
                continue
