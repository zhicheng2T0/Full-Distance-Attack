...
def get_det_loss(model, p_img, lab_batch, args, kwargs):
    valid_num = 0
    ...
    if kwargs['name'] == 'yolov2':
        output = model(p_img)
        #print(output.shape) #(bs,425,13,13), 425=5*(80+5)
        all_boxes_t = [utils.get_region_boxes_general(output, model, conf_thresh=args.conf_thresh, name=kwargs['name'])]
        '''
        for i in range(len(all_boxes_t[0])):
            print(i,all_boxes_t[0][i].shape)
        return
        0 torch.Size([16, 7])
        1 torch.Size([103, 7])
        2 torch.Size([75, 7])
...
def random_crop(cloth, crop_size, pos=None, crop_type=None, fill=0):
    ...
    if crop_type is None:
        ...
        #print(cloth.shape)
        #print(r_w,crop_size[0],r_h,crop_size[1])
        #print(cloth_expanded.shape)
        #torch.Size([1, 3, 1800, 1800])
        patch = cloth_expanded[:, :, r_w:r_w + crop_size[0], r_h:r_h + crop_size[1]]
        #patch=cloth_expanded[:, :, r_w:r_w + crop_size[0], :]
    else:
        raise ValueError
