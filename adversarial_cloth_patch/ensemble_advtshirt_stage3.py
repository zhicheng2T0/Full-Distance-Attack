...
if args.net=='yolov2':
    ...
    fp16_cfg = cfg3.get('fp16', None)
    if fp16_cfg is not None:
        wrap_fp16_model(model3)
    checkpoint3 = load_checkpoint(model3, args.checkpoint3, map_location='cpu')
    if 'CLASSES' in checkpoint3.get('meta', {}):
        model3.CLASSES = checkpoint3['meta']['CLASSES']
    else:
        model2.CLASSES = dataset.CLASSES

    model3 = build_dp(model3, cfg3.device, device_ids=cfg3.gpu_ids)
    ...
    model_type_list=[0,1,1]
    model_name_list=['pvt-retina','res-maskrcnn','Swin']
    conf_thresh_list=[0.3,0.3,0.3]
    nms_thresh_list=[0.7,0.7,0.7]
    iou_thresh_list=[0.5,0.5,0.5]
    model_imgsize_list=[800,416,416]
    optimize_index=2
...
def train_patch_stage1():
    ...
    img_path = 'replace_with_your_npy_file'#load the last stage 2 patch from ensemble_advtshirt_stage2.py
    ...
    for epoch in range(1, 600 + 1):
        ...
        for i_batch in range(total_image_num):
            cur_model_index=1+int(np.random.rand()*2)
...
def train_patch():
    ...
    data_base=data_base='/home/chengzhi/data'
    physical_loader=PhysicalLoader(data_base)
    data_subdircts=['2m','6m','12m','18m','24m','32m','40m']
    data_subdircts_train=['normal person']
    scale_bg_range=0.2
